...
async function loadPopup() {
  ...
  // ✅ Load individual gesture states
  document.getElementById("gesture-back").checked = siteRule.back !== false;
  document.getElementById("gesture-forward").checked =
    siteRule.forward !== false;
  document.getElementById("gesture-reload").checked = siteRule.reload !== false;
  document.getElementById("gesture-up").checked = siteRule.up !== false;
  ...
  siteBtn.addEventListener("click", async () => {
    ...
    cfg.siteRules[host].enabled = !disabled;
    await chrome.storage.local.set({ config: cfg });
    sendCmd({ action: "reload-config" });
    updateStatus(disabled, cfg.enabled);
  });

  // ✅ NEW: Individual gesture toggles
  const gestureCheckboxes = document.querySelectorAll(".gesture-checkbox");
  gestureCheckboxes.forEach((checkbox) => {
    checkbox.addEventListener("change", async () => {
      ...
      // Update specific gesture
      const gesture = checkbox.id.replace("gesture-", "");
      cfg.siteRules[host][gesture] = checkbox.checked;
  ...
  // ✅ NEW: Quick toggle horizontal (back + forward)
  document
    .getElementById("toggle-horizontal")
    .addEventListener("click", async () => {
      ...
      const currentState =
        cfg.siteRules[host].back && cfg.siteRules[host].forward;
      const newState = !currentState;

      cfg.siteRules[host].back = newState;
      cfg.siteRules[host].forward = newState;

      document.getElementById("gesture-back").checked = newState;
      document.getElementById("gesture-forward").checked = newState;

      await chrome.storage.local.set({ config: cfg });
      sendCmd({ action: "reload-config" });
    });

  // ✅ NEW: Quick toggle vertical (reload + up)
  document
    .getElementById("toggle-vertical")
    .addEventListener("click", async () => {
      if (!cfg.siteRules[host]) {
        cfg.siteRules[host] = {
          enabled: true,
      ...
      const currentState = cfg.siteRules[host].reload && cfg.siteRules[host].up;
      const newState = !currentState;

      cfg.siteRules[host].reload = newState;
      cfg.siteRules[host].up = newState;

      document.getElementById("gesture-reload").checked = newState;
      document.getElementById("gesture-up").checked = newState;
...
function updateStatus(disabled, globallyEnabled) {
  ...
  if (!globallyEnabled) {
    ...
    if (disabled) {
      ...
      gestureControls.classList.remove("hidden"); // ✅ Show controls
    }
  }
}
